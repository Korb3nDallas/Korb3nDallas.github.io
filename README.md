...
<head>
    ...
    <title>Cours Interactif - Réseaux & TCP/IP</title>
    <style>
        :root {
            --bg: #0f172a;
            --card-bg: #1e293b;
...
<nav>
    <div class="nav-container">
        <div class="logo">🚀 Réseau Master</div>
        <div class="tabs">
            <button class="tab-btn active" onclick="switchTab('osi')">Modèle OSI</button>
...
<main>
    <div id="osi" class="section active">
        <h2>Le Modèle OSI (7 Couches)</h2>
        <p style="margin-bottom: 20px; color:#94a3b8;">Clique sur une couche pour voir les détails. Rappelle-toi : "Pour Le Réseau Tout Se Passe Automatiquement" (7 vers 1).</p>
    ...
    <div id="cmds" class="section">
        ...
        <p style="color:#94a3b8;">Clique sur une commande pour voir ce qu'elle fait réellement.</p>
        ...
        <div class="terminal-window">
            ...
            <div class="terminal-body" id="terminalOutput">
                <div class="line">Bienvenue dans le simulateur réseau.</div>
                <div class="line">Sélectionnez une commande ci-dessus pour commencer...<span class="typing"></span></div>
            </div>
        </div>
    </div>
    ...
    <div id="ips" class="section">
        ...
        <div class="def-grid">
            <div class="def-card" style="border-color: #f59e0b;">
                ...
                <p style="font-size:0.8rem; margin-top:10px; color:#9ca3b8;">Note: 127.x.x.x est réservé pour le Loopback (localhost).</p>
            </div>

            <div class="def-card" style="border-color: #10b981;">
                <div class="def-title">Adresses Privées (RFC 1918)</div>
                <p>Ces adresses ne sont pas routables sur Internet (utilisées en LAN).</p>
            ...
            <div class="def-card" style="border-color: #ec4899;">
                <div class="def-title">Différences Matérielles</div>
                <p><strong>Hub :</strong> "Bête". Envoie les données à tout le monde (Broadcast). Couche 1.</p>
                <p><strong>Switch :</strong> "Intelligent". Utilise l'adresse MAC pour envoyer au bon destinataire. Couche 2.</p>
                <p><strong>Routeur :</strong> "Chef". Connecte différents réseaux (LAN/WAN) via IP. Couche 3.</p>
            </div>
            ...
            <div class="def-card" style="border-color: #8b5cf6;">
                ...
                <div style="margin-bottom:10px">
                    ...
                    Connexion établie (3-way handshake). Vérifie que les paquets arrivent. Plus lent.<br>
                    <em>Ex: Web, Email, Fichiers.</em>
                </div>
                <div>
                    <strong style="color:#8b5cf6">UDP (Rapide)</strong><br>
                    Pas de connexion. "Fire and forget". Si un paquet est perdu, tant pis. Temps réel.<br>
...
<script>
    // --- DONNÉES ---
    const osiLayers = [
        { n:7, name:"Application", color:"#ec4899", pdu:"Donnée", dev:"PC / Proxy", desc:"Interface directe avec l'utilisateur (Navigateur, Client Mail)." },
        { n:6, name:"Présentation", color:"#d946ef", pdu:"Donnée", dev:"Gateway", desc:"Traducteur. S'occupe du formatage, chiffrement (SSL/TLS) et compression." },
        { n:5, name:"Session", color:"#a855f7", pdu:"Donnée", dev:"Gateway", desc:"Gendarme. Ouvre, gère et ferme les sessions entre applications." },
        { n:4, name:"Transport", color:"#8b5cf6", pdu:"Segment", dev:"Firewall", desc:"Livreur. Choisit le protocole (TCP/UDP) et découpe les données. Gère les ports." },
        { n:3, name:"Réseau", color:"#3b82f6", pdu:"Paquet", dev:"Routeur", desc:"GPS. Adressage IP logique et détermination du meilleur chemin (Routage)." },
        { n:2, name:"Liaison", color:"#0ea5e9", pdu:"Trame", dev:"Switch", desc:"Contrôle Qualité. Adressage MAC physique et détection d'erreurs locales." },
        { n:1, name:"Physique", color:"#06b6d4", pdu:"Bit", dev:"Câble / Hub", desc:"La Route. Transmission brute des 0 et 1 sous forme de signaux (électrique/optique)." }
    ];

    const portsData = [
        { p:20, n:"FTP", d:"Transfert de fichiers (Data)", c:"web" },
        { p:21, n:"FTP", d:"Transfert de fichiers (Commandes)", c:"web" },
        { p:22, n:"SSH", d:"Connexion sécurisée à distance (Linux)", c:"infra" },
        { p:23, n:"Telnet", d:"Connexion à distance non sécurisée (Obsolète)", c:"infra" },
        ...
        { p:80, n:"HTTP", d:"Web non sécurisé", c:"web" },
        { p:110, n:"POP3", d:"Réception d'emails (Télécharge tout)", c:"mail" },
        { p:143, n:"IMAP", d:"Réception d'emails (Synchro serveur)", c:"mail" },
        { p:443, n:"HTTPS", d:"Web sécurisé (Cadenas)", c:"web" },
        { p:3389, n:"RDP", d:"Bureau à distance Windows", c:"infra" }
    ];

    const commands = [
        { c:"ping 8.8.8.8", r:"Envoi de 4 paquets ICMP vers 8.8.8.8...\nRéponse de 8.8.8.8 : octets=32 temps=12ms TTL=115\n\n> Vérifie si une machine est joignable." },
        { c:"ipconfig /all", r:"Configuration IP de Windows...\nAdresse IPv4. . . . . . . : 192.168.1.25\nMasque de sous-réseau . : 255.255.255.0\nPasserelle par défaut . : 192.168.1.1\nAdresse Physique (MAC)  : AB-CD-EF-12-34-56\n\n> Affiche TOUS les détails de la carte réseau." },
        { c:"tracert google.com", r:"Détermination de l'itinéraire vers google.com...\n1    <1 ms    192.168.1.1 (Box)\n2    15 ms    80.12.xx.xx (FAI)\n3    18 ms    google-router.com\n\n> Affiche chaque saut (routeur) traversé par le paquet." },
    ...
    osiLayers.forEach(l => {
        ...
        el.innerHTML = `
            ...
            <div class="osi-content">
                <p><strong>Rôle :</strong> ${l.desc}</p>
                <div style="margin-top:10px">
                    <span class="tag tag-pdu">PDU: ${l.pdu}</span>
                    <span class="tag tag-dev">Matériel: ${l.dev}</span>
                </div>
            </div>
        `;
        el.onclick = () => {
    ...
    function runCmd(cmdObj) {
        ...
        // Effet de délai pour simuler le calcul
        setTimeout(() => {
